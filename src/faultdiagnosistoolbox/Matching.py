    ...
    def __init__(self, Xmodel, eqs_in):
        ...
        for hc in reversed(dm.M0):
            ...
            if not intEdge and not derEdge: # Algebraic component
                self.matching.append(HallComponentMatching('algebraic', eqs[eqi],vi))
            elif len(eqi)==1 and derEdge: # Trivial derivative component
                self.matching.append(HallComponentMatching('der', eqs[eqi],vi,derState=vi))
                derCausal=True
            elif len(eqi)==1 and intEdge: # Trivial integral component
                self.matching.append(HallComponentMatching('int', eqs[eqi],vi,intState=vi))
                intCausal=True
                ...
                if Gamma_mixed.matchType is 'int':
                    ...
                    derCausal = True
                    intCausal = True
                self.matching.append(HallComponentMatching(Gamma_mixed.matchType, eqs[Gamma_mixed.row], Gamma_mixed.col,
                                                               derState=Gamma_mixed.derState, intState=Gamma_mixed.intState))
...
def MixedCausalityMatching( hallComponent, X ):
    ...
    G = (hallComponent.row, hallComponent.col, X[hallComponent.row,:][:,hallComponent.col])
    ...
    if derCausal and intCausal:
        ...
        gamma_type = 'int'
    return HallComponentMatching(gamma_type,Gamma[:,0], Gamma[:,1],derState = derState, intState=intState)
