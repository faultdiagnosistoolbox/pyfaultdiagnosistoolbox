...
modeldef["rels"] = [
    ...
    -W_af + W_af_fun(p_amb, p_af, plin_af, H_af, T_amb),
    ...
    -dmdt_af + W_af - W_c,
    -dTdt_af
    + (W_af * cv_air * (T_amb - T_af) + R_air * (T_amb * W_af - T_af * W_c))
    / (m_af * cv_air),
    ...
    -dmdt_c + W_c - W_ic,
    -dTdt_c
    + (W_c * cv_air * (T_cout - T_c) + R_air * (T_cout * W_c - T_c * W_ic))
    / (m_c * cv_air),
    ...
    -dTdt_ic
    + (
        W_ic * cv_air * (T_fwd_flow_ic - T_ic)
        + R_air * (T_fwd_flow_ic * W_ic - T_ic * W_th)
    )
    / (m_ic * cv_air),
    ...
    -dTdt_im
    + (W_th * cv_air * (T_imcr - T_im) + R_air * (T_imcr * W_th - T_im * W_ac))
    / (m_im * cv_air),
    ...
    -dmdt_em + W_e - W_twg,
    -dTdt_em
    + (W_e * cv_exh * (T_ti - T_em) + R_exh * (T_ti * W_e - T_em * W_twg))
    / (m_em * cv_exh),
    ...
    -dmdt_t + W_twg - W_es,
    -dTdt_t
    + (W_twg * cv_exh * (T_turb - T_t) + R_exh * (T_turb * W_twg - T_t * W_es))
    / (m_t * cv_exh),
    ...
    + max_fun(
        ...
        T_c
        - max_fun(
            TOL, (T_c - T_amb) * (Cic_1 + (T_amb + T_c) * Cic_2 / 2 + W_ic * Cic_3)
        ),
    ),
    -Aeff_th
    + A_0
    + A_1 * (alpha_th * 86 / 100 + 4) * sym.pi / 180
    + A_2 * ((alpha_th * 86 / 100 + 4) * sym.pi / 180) ** 2,
    -W_ac + eta_vol * V_D * 1 / (2 * sym.pi) * omega_e * p_im / (n_r * R_air * T_im),
    -T_in + T_im - CEva_cool * (1 / lambda_af - 1),
    -eta_vol
    + C_eta_vol
    * ((r_c - (p_em / p_im) ** (1 / gamma_exh)) * T_im)
    / ((r_c - 1) * T_in),
    -C_eta_vol
    + a1 * (max_fun(p_im, 2.5e4) * 1e-3) ** 4
    + a2 * (max_fun(p_im, 2.5e4) * 1e-3) ** 3
    + a3 * (max_fun(p_im, 2.5e4) * 1e-3) ** 2
    + a4 * (max_fun(p_im, 2.5e4) * 1e-3)
    + a5,
    ...
    -FMEP
    + xi_aux
    * ((0.464 + 0.0072 * S_p ** (1.8)) * PI_bl * 10**5 + 0.0215 * BMEP)
    * (0.075 / B) ** 0.5,
    ...
    -PSI_c
    + (2 * cp_air * ((PI_c ** ((gamma_air - 1) / gamma_air)) - 1) * T_af) / (U_c**2),
    ...
    -eta_c
    + eta_c_fun(
        eta_cmax, eta_cmin, W_ccorr, W_ccorrmax, PI_c, PI_cmax, Q_c11, Q_c12, Q_c22
    ),
    ...
    + max_fun(
        ...
        min_fun(
            1 - TOL,
            (
                a_eta_t_fun(omega_tc) * sym.sqrt(T_em) * W_t / (p_em * 1e-3)
                + b_eta_t_fun(omega_tc)
            )
            / dh_is,
        ),
    ),
    ...
    -domegadt_tc + 1 / J_tc * (Tq_t - Tq_c - omega_tc * xi_fric_tc),
...
LiU_ICE_model = fdt.DiagnosisModel(modeldef, name="LiU_ICE model")
